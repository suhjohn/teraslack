...
const (
	defaultCLIManifestBaseURL = "https://downloads.teraslack.ai/teraslack/cli"
)
...
func (c *CLI) runLifecycle(ctx context.Context, name string, args []string, output string, stdout, stderr io.Writer) int {
	switch name {
	case "version":
...
func (c *CLI) runVersion(args []string, output string, stdout, stderr io.Writer) int {
	...
	if output == "json" {
		if err := writeOutput(stdout, map[string]any{
			"name":    "teraslack",
	...
	_, err := fmt.Fprintf(stdout, "teraslack %s\n", strings.TrimSpace(Version))
	if err != nil {
		fmt.Fprintf(stderr, "write output: %v\n", err)
		return 1
	}
...
func (c *CLI) runUpdate(ctx context.Context, args []string, stdout, stderr io.Writer) int {
...
func (c *CLI) runUninstall(args []string, stdout, stderr io.Writer) int {
	var keepConfig bool
...
func downloadFile(ctx context.Context, sourceURL, destination string) error {
...
func extractBinaryFromArchive(archivePath, tmpDir, binaryName string) (string, error) {
...
func extractBinaryFromZip(archivePath, tmpDir, binaryName string) (string, error) {
...
func extractBinaryFromTarGz(archivePath, tmpDir, binaryName string) (string, error) {
...
func scheduleWindowsReplace(sourcePath, targetPath string) error {
	stagedPath := targetPath + ".new"
	input, err := os.Open(sourcePath)
	if err != nil {
		return err
...
func scheduleWindowsUninstall(exePath, configPath, binDir, installRoot string, keepConfig bool) error {
	configRemoval := ""
	if !keepConfig {
...
func rewriteShellProfile(path, binDir string) error {
...
func stripInstallerPathBlock(content, binDir string) string {
