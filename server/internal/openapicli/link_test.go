...
func TestRunUnlinkRemovesCurrentDirectoryLink(t *testing.T) {
	configDir := filepath.Join(t.TempDir(), "teraslack-config")
	t.Setenv("TERASLACK_CONFIG_DIR", configDir)
	t.Setenv("TERASLACK_CONFIG_FILE", "")
	...
	stdout.Reset()
	stderr.Reset()
	if code := cli.runUnlink(nil, "json", &stdout, &stderr); code != 0 {
		t.Fatalf("runUnlink() code = %d, stderr = %s", code, stderr.String())
	}

	var removed linkCommandResponse
	if err := json.Unmarshal(stdout.Bytes(), &removed); err != nil {
		t.Fatalf("Unmarshal(unlink output) error = %v", err)
	}
	if removed.CurrentPath != canonicalProjectDir {
		t.Fatalf("removed.CurrentPath = %q, want %q", removed.CurrentPath, canonicalProjectDir)
	}
	if removed.LinkedPath != canonicalProjectDir {
		t.Fatalf("removed.LinkedPath = %q, want %q", removed.LinkedPath, canonicalProjectDir)
	}
	if removed.ConversationID != conversationID {
		t.Fatalf("removed.ConversationID = %q, want %q", removed.ConversationID, conversationID)
	}

	stdout.Reset()
	stderr.Reset()
	if code := cli.runLink(nil, "json", &stdout, &stderr); code == 0 {
		t.Fatalf("expected runLink(show) to fail after unlink")
	}
}

func TestRunUnlinkRemovesNearestParentDirectoryLink(t *testing.T) {
	configDir := filepath.Join(t.TempDir(), "teraslack-config")
	t.Setenv("TERASLACK_CONFIG_DIR", configDir)
	t.Setenv("TERASLACK_CONFIG_FILE", "")
	...
	if removed.CurrentPath != canonicalSubDir {
		t.Fatalf("removed.CurrentPath = %q, want %q", removed.CurrentPath, canonicalSubDir)
	}
	if removed.LinkedPath != canonicalRepoDir {
		t.Fatalf("removed.LinkedPath = %q, want %q", removed.LinkedPath, canonicalRepoDir)
	}
	if removed.ConversationID != conversationID {
		t.Fatalf("removed.ConversationID = %q, want %q", removed.ConversationID, conversationID)
	}
