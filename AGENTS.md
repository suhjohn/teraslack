...
# Server OpenAPI Sync Rule

- Canonical HTTP API contract: `server/api/openapi.yaml`
- For product API routes, update `server/api/openapi.yaml` first and regenerate `server/internal/api/openapi.gen.go` in the same change.
- Keep these in sync:
  - documented paths, request bodies, response schemas, and auth requirements in `server/api/openapi.yaml`
  - generated server bindings in `server/internal/api/openapi.gen.go`
  - handwritten adapter methods in `server/internal/handler/openapi_server.go`
  - CLI-discoverable API surface in `server/internal/openapicli/cli.go`
- Treat `server/internal/handler/router.go` manual route registration as an exception path for non-OpenAPI endpoints only, such as helper/auth bootstrap routes that are intentionally outside the public API contract.
...
# UI Picker Principles

For dropdowns, comboboxes, and picker menus in `agent-manager-web`, follow the same design language used by the shared picker components such as `variant-combobox.tsx`, `model-combobox.tsx`, and related menu primitives:

- Use custom popover-based menus, not native `select` or `datalist`, when the menu needs to match product styling.
- Picker surfaces should read as one consistent system: `rounded-xl`, compact inner padding, `glass-menu`, and `glass-shadow`.
- Search inputs inside menus should feel embedded into the menu surface, with low-contrast borders and quiet focus treatment.
- Option rows should be compact and text-first. Prefer one primary label per option. Do not show duplicate value-plus-label pairs when they communicate the same thing.
- Use section labels sparingly and consistently: small uppercase labels that orient the user without adding noise.
- Selected state should be obvious through a checkmark and the shared blue active/hover treatment, not heavy borders or noisy badges.
- Trigger buttons should stay compact, neutral, and readable. They should feel like a lightweight control, not a separate card.
- Secondary metadata inside options should only appear when it changes meaning materially, such as ownership, scope, or default status.
